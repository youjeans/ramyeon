...
$sql = "SELECT r.`라면이름`, n.`탄수화물` 
        FROM ramyun r
        JOIN nutrition n ON r.`라면 번호` = n.`라면 번호`
        ORDER BY n.`탄수화물` $sortOrder";
...
<table id="ramenTable">
    ...
    <tbody>
        ...
        foreach ($ramyunData as $row) {
            ...
            $carbo = isset($row["탄수화물"]) ? $row["탄수화물"] : "Undefined";
            echo "<tr><td>" . $ramyun_name . "</td><td>" . $carbo . "</td></tr>";
        }
        ?>
    </tbody>
</table>
...
<script>
    function sortInstantNoodles() {
        ...
        window.location.href = 'demo_ca.php?sort=' + sortOrder;
    }
</script>
