...
$sql = "SELECT r.`라면이름`, n.`지방`
        FROM ramyun r
        JOIN nutrition n ON r.`라면 번호` = n.`라면 번호`
        ORDER BY n.`지방` $sortOrder";
...
<table id="ramenTable">
    ...
    <tbody>
        ...
        foreach ($ramyunData as $row) {
            ...
            $fat = isset($row["지방"]) ? $row["지방"] : "Undefined";
            echo "<tr><td>" . $ramyun_name . "</td><td>" . $fat . "</td></tr>";
        }
        ?>
    </tbody>
</table>
...
<script>
    function sortInstantNoodles() {
        ...
        window.location.href = 'demo_fat.php?sort=' + sortOrder;
    }
</script>
