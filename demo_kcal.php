...
$sql = "SELECT r.`라면이름`, n.`칼로리`
        FROM ramyun r
        JOIN nutrition n ON r.`라면 번호` = n.`라면 번호`
        ORDER BY n.`칼로리` $sortOrder";
...
<table id="ramenTable">
    ...
    <tbody>
        ...
        foreach ($ramyunData as $row) {
            ...
            $kcal = isset($row["칼로리"]) ? $row["칼로리"] : "Undefined";
            echo "<tr><td>" . $ramyun_name . "</td><td>" . $kcal . "</td></tr>";
        }
        ?>
    </tbody>
</table>
...
<script>
    function sortInstantNoodles() {
        ...
        window.location.href = 'demo_kcal.php?sort=' + sortOrder;
    }
</script>
