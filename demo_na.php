...
$sql = "SELECT r.`라면이름`, n.`나트륨` 
        FROM ramyun r
        JOIN nutrition n ON r.`라면 번호` = n.`라면 번호`
        ORDER BY n.`나트륨` $sortOrder";
...
<table id="ramenTable">
    ...
    <tbody>
        ...
        foreach ($ramyunData as $row) {
            ...
            $nacl = isset($row["나트륨"]) ? $row["나트륨"] : "Undefined";
            echo "<tr><td>" . $ramyun_name . "</td><td>" . $nacl . "</td></tr>";
        }
        ?>
    </tbody>
</table>
...
<script>
    function sortInstantNoodles() {
        ...
        window.location.href = 'demo_na.php?sort=' + sortOrder;
    }
</script>
