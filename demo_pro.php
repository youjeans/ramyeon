...
$sql = "SELECT r.`라면이름`, n.`단백질`
        FROM ramyun r
        JOIN nutrition n ON r.`라면 번호` = n.`라면 번호`
        ORDER BY n.`단백질` $sortOrder";
...
<table id="ramenTable">
    ...
    <tbody>
        ...
        foreach ($ramyunData as $row) {
            ...
            $prot = isset($row["단백질"]) ? $row["단백질"] : "Undefined";
            echo "<tr><td>" . $ramyun_name . "</td><td>" . $prot . "</td></tr>";
        }
        ?>
    </tbody>
</table>
...
<script>
    function sortInstantNoodles() {
        ...
        window.location.href = 'demo_pro.php?sort=' + sortOrder;
    }
</script>
